...
const TestProfileComponent = (props: BlockProps) => {
...
describe("<Profile />", () => {
    test("renders summary view correctly", async () => {
        ...
        const wrapper = render(<TestProfileComponent contentEntry={mockProfile} />);
        expect(wrapper).toBeTruthy();

        const name = wrapper.container.querySelector("#profileName")?.textContent;
        const coverImg = wrapper.container.querySelector("#profileImage")?.getAttribute("src");
        const body = wrapper.container.querySelector("#profileBody")?.textContent;
    ...
    test("renders full view correctly", () => {
        ...
        const wrapper = render(<TestProfileComponent detail={true} contentEntry={mockProfile} />);
        expect(wrapper).toBeTruthy();

        const title = wrapper.container.querySelector("#title")?.textContent;
        const coverImg = wrapper.container.querySelector("#profileImage")?.getAttribute("src");
        const body = wrapper.container.querySelector("#body")?.textContent;
        const otherImg = wrapper.container.querySelector("#otherImages")?.getAttribute("src");
        const otherImgDescription = wrapper.container.querySelector("#otherImgDescription")?.textContent;

        const imgLength = wrapper.container.querySelectorAll("#otherImages").length;
        const otherImgDescriptionLength = wrapper.container.querySelectorAll("#otherImgDescription").length;
